...
const ALL_REPORTS: ReportDef[] = [
  {
    key: 'overall', title: UI_TEXT.OverallFarmPnL, desc: 'Revenue, cost & net profit', icon: <TrendingUp size={18} />,
  ...
  {
    key: 'season', title: UI_TEXT.SeasonPnL, desc: 'Per-season profitability', icon: <BarChart3 size={18} />,
  ...
  {
    key: 'crop', title: UI_TEXT.CropwisePnL, desc: 'Every crop with margin', icon: <TrendingUp size={18} />,
  ...
  {
    key: 'nursery', title: UI_TEXT.NurseryBatchPnL, desc: 'Batch & nursery totals', icon: <Sprout size={18} />,
  ...
  {
    key: 'ledger', title: UI_TEXT.FundingLiabilities, desc: 'Capital, loans, shop transfers', icon: <BookOpen size={18} />,
  ...
  {
    key: 'vouchers', title: UI_TEXT.VoucherRegister, desc: 'All payment vouchers', icon: <FileText size={18} />,
  ...
  {
    key: 'expenses', title: UI_TEXT.ExpenseLogReport, desc: 'All classified expenses', icon: <FileText size={18} />,
  ...
  {
    key: 'capex', title: UI_TEXT.CAPEXReport, desc: 'Fixed assets & infrastructure spend', icon: <Hammer size={18} />,
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat label={UI_TEXT.OverallProfit} value={LKR(farmOverallPnL(filteredData).profit)} tone="green" icon={<TrendingUp size={18} />} />
        <Stat label={UI_TEXT.NurseryNet} value={LKR(nurseryTotals(filteredData).netProfit)} tone="blue" icon={<Sprout size={18} />} />
        <Stat label={UI_TEXT.VouchersLabel} value={String(filteredData.vouchers.length)} sub="Filtered" tone="yellow" icon={<FileText size={18} />} />
        <Stat label={UI_TEXT.Records} value={String(filteredData.crops.length + filteredData.nurseryBatches.length + filteredData.workers.length + filteredData.vouchers.length + filteredData.ledger.length)} sub="In database" tone="neutral" icon={<Database size={18} />} />
      </div>

      {/* Reports grid */}
      <Card className="p-5">
        <SectionTitle title={UI_TEXT.ReportsExports} subtitle="One-click export to Excel (CSV) and PDF (A5 print) with multi-criteria filtering (එක් ක්ලික් එක්සෙල් (CSV) හා PDF (A5 මුද්‍රණය) අපනයනය බහු-නිර්ණායක පෙරහන් සහිතව)" icon={<FileSpreadsheet size={18} />}
          action={<Button size="sm" variant="outline" icon={<Download size={14} />} onClick={() => exportAllZip(data)}>{UI_TEXT.ExportAll}</Button>} />

        {/* Filter bar */}
        <div className="grid sm:grid-cols-3 gap-3 mb-4 p-3 rounded-xl bg-neutral-50 border border-neutral-100">
          <Select label={UI_TEXT.FilterByYear} value={filters.year} onChange={(e) => setFilters({ ...filters, year: e.target.value })}>
            <option value="">{UI_TEXT.AllYears}</option>
            {years.map((y) => <option key={y} value={y}>{y}</option>)}
          </Select>
          <Select label={UI_TEXT.FilterByCrop} value={filters.cropId} onChange={(e) => setFilters({ ...filters, cropId: e.target.value })}>
            <option value="">{UI_TEXT.AllCrops}</option>
            {data.crops.map((c: Crop) => <option key={c.id} value={c.id}>{c.name} · {c.plot}</option>)}
          </Select>
          <Select label={UI_TEXT.FilterByBatch} value={filters.batchId} onChange={(e) => setFilters({ ...filters, batchId: e.target.value })}>
            <option value="">{UI_TEXT.AllBatches}</option>
            ...
            return (
              <div key={r.key} className={`p-4 rounded-xl border transition group cursor-pointer ${selectedReport === r.key ? 'border-primary-400 shadow-card bg-primary-50/30' : 'border-neutral-200 hover:border-primary-300 hover:shadow-card'}`} onClick={() => setSelectedReport(selectedReport === r.key ? null : r.key)}>
                ...
                <div className="mt-3 flex gap-2" onClick={(e) => e.stopPropagation()}>
                  <Button size="sm" variant="outline" icon={<FileSpreadsheet size={13} />} onClick={() => exportCSV(`${r.key}_report.csv`, rows)}>{UI_TEXT.Excel}</Button>
                  <Button size="sm" variant="ghost" icon={<FileText size={13} />} onClick={() => exportPDFviaPrint(r.title, rows)}>{UI_TEXT.PDF}</Button>
                </div>
              </div>
            );
          })}
          ...
          <div className="mt-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" icon={<FileSpreadsheet size={13} />} onClick={() => exportCSV(`${activeReport.key}_report.csv`, previewRows)}>Export Excel (එක්සෙල් අපනයනය)</Button>
                <Button size="sm" variant="ghost" icon={<FileText size={13} />} onClick={() => exportPDFviaPrint(activeReport.title, previewRows)}>Export PDF (PDF අපනයනය)</Button>
              </div>
            </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Import */}
        <Card className="p-5">
          <SectionTitle title={UI_TEXT.BulkImport} subtitle={UI_TEXT.MigrateHistorical} icon={<Upload size={18} />} />
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <Select label={UI_TEXT.Target} value={importTarget || ''} onChange={(e) => setImportTarget(e.target.value as ImportTarget)}>
              <option value="">{UI_TEXT.SelectTarget}</option>
              {IMPORT_TARGETS.map((t) => <option key={t.key} value={t.key}>{t.label}</option>)}
            </Select>
            <Select label={UI_TEXT.Mode} value={importMode} onChange={(e) => setImportMode(e.target.value as 'append' | 'replace')}>
              <option value="append">{UI_TEXT.AppendExisting}</option>
              <option value="replace">{UI_TEXT.ReplaceAll}</option>
            </Select>
          </div>
          <div className="mt-4">
            ...
            <div className="flex gap-2">
              <Button size="md" icon={<Upload size={15} />} disabled={!importTarget} onClick={() => fileRef.current?.click()}>{UI_TEXT.ChooseCSV}</Button>
              <Button size="md" variant="outline" icon={<Download size={15} />} disabled={!importTarget} onClick={() => { if (!importTarget) return; downloadFile(`${importTarget}_template.csv`, sampleCSV(importTarget), 'text/csv;charset=utf-8;'); }}>{UI_TEXT.DownloadTemplate}</Button>
            </div>
          </div>
        ...
        <Card className="p-5">
          <SectionTitle title={UI_TEXT.AIFinancialAssistant} subtitle={UI_TEXT.NaturalLanguageQueries} icon={<Sparkles size={18} />} action={<Button size="sm" onClick={() => setAiOpen(true)}>{UI_TEXT.Open}</Button>} />
...
function ReportPreviewTable({ rows }: { rows: Record<string, string | number>[] }) {
  if (!rows.length) return <div className="py-6 text-center text-sm text-neutral-500">{UI_TEXT.NoDataFilters}</div>;
...
function AIModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal open onClose={onClose} title={UI_TEXT.AIFinancialAssistant} size="lg">
      ...
      <form onSubmit={(e) => { e.preventDefault(); run(q); }} className="flex gap-2">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={UI_TEXT.AskQuestion} className="flex-1 px-3 py-2 rounded-lg border border-neutral-300 text-sm focus:border-accent-500 focus:ring-2 focus:ring-accent-100 outline-none" />
        <Button type="submit">Ask (අසන්න)</Button>
      </form>
      {out && (
