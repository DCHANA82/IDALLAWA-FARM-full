...
export function SettingsModule() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5">
        <SectionTitle title={UI_TEXT.FarmProfile} subtitle="Organization details, contact info, photo & logo (සංවිධාන විස්තර, සබඳතා තොරතුරු, ඡායාරූප හා ලාඤ්ඤය)" icon={<Building2 size={18} />} />
        <div className="flex flex-col sm:flex-row gap-6">
          ...
          <div className="flex-1 space-y-3">
            <div className="grid sm:grid-cols-2 gap-4">
              <Input label="Farm / company name (ගොවිපළ / සමාගමේ නම)" value={farmName} onChange={(e) => setFarmName(e.target.value)} />
              <Input label={UI_TEXT.Owner} value={owner} onChange={(e) => setOwner(e.target.value)} />
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <Input label={UI_TEXT.Phone} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+94 77 123 4567" />
              <Input label={UI_TEXT.Email} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="farm@example.com" />
            </div>
            <Input label={UI_TEXT.Address} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="No. 12, Farm Road, Idallawa, Sri Lanka" />
            <div className="flex items-center gap-3">
              <Button icon={<Save size={15} />} onClick={saveProfileData}>Save profile (පැතිකඩ සුරකින්න)</Button>
              {profilePhoto && <Button variant="ghost" size="sm" onClick={() => setProfilePhoto('')}>Remove photo (ඡායාරූපය ඉවත් කරන්න)</Button>}
              {logo && <Button variant="ghost" size="sm" onClick={() => setLogo('')}>Remove logo (ලාඤ්ඤය ඉවත් කරන්න)</Button>}
            </div>
          </div>
        </div>
      </Card>
      ...
      {/* Branding & Appearance */}
      <Card className="p-5">
        <SectionTitle title={UI_TEXT.BrandingAppearance} subtitle="Customize background images with brightness, overlay & blur controls (දීප්තිය, ආවරණය හා අවුස්ථාව පාලනයක් සහිතව පසුබිම් රූප අභිරුචිකරණය)" icon={<Palette size={18} />} />
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="space-y-3">
            <div className="text-sm font-600 text-neutral-700">Login Page Background (පිවිසුම් පිටු පසුබිම)</div>
            ...
            <SliderControl label={UI_TEXT.Brightness} value={loginBrightness} min={0.2} max={1} step={0.05} onChange={setLoginBrightness} format={(v) => `${Math.round(v * 100)}%`} />
            <SliderControl label={UI_TEXT.DarkOverlay} value={loginOverlay} min={0} max={0.9} step={0.05} onChange={setLoginOverlay} format={(v) => `${Math.round(v * 100)}%`} />
            <SliderControl label={UI_TEXT.BlurEffect} value={loginBlur} min={0} max={20} step={1} onChange={setLoginBlur} format={(v) => v === 0 ? 'Off' : `${v}px`} />
          ...
          <div className="space-y-3">
            <div className="text-sm font-600 text-neutral-700">Dashboard Background (උපකරණ පුවරු පසුබිම)</div>
            ...
            <SliderControl label={UI_TEXT.Brightness} value={dashBrightness} min={0.2} max={1} step={0.05} onChange={setDashBrightness} format={(v) => `${Math.round(v * 100)}%`} />
            <SliderControl label={UI_TEXT.Visibility} value={dashOverlay} min={0} max={1} step={0.05} onChange={setDashOverlay} format={(v) => `${Math.round(v * 100)}%`} />
            <SliderControl label={UI_TEXT.BlurEffect} value={dashBlur} min={0} max={20} step={1} onChange={setDashBlur} format={(v) => v === 0 ? 'Off' : `${v}px`} />
        ...
        <div className="mt-4">
          <Button icon={<Save size={15} />} onClick={saveProfileData}>{UI_TEXT.SaveAppearance}</Button>
        </div>
      </Card>
      ...
      {(data.address || data.phone || data.email) && (
        <Card className="p-5">
          <SectionTitle title="Contact Information (සබඳතා තොරතුරු)" subtitle="Current contact details on file (ගොනුවේ වර්තමාන සබඳතා තොරතුරු)" icon={<MapPin size={18} />} />
          <div className="grid sm:grid-cols-3 gap-4">
            <ContactItem icon={<MapPin size={16} />} label={UI_TEXT.Address} value={data.address} />
            <ContactItem icon={<Phone size={16} />} label={UI_TEXT.Phone} value={data.phone} />
            <ContactItem icon={<Mail size={16} />} label={UI_TEXT.Email} value={data.email} />
          </div>
        </Card>
      )}
      ...
      <Card className="p-5">
        <SectionTitle title="My Credentials (මාගේ අක්තපත්‍ර)" subtitle="Update your own login username and password (ඔබේ පිවිසුම් පරිශීලක නාමය හා මුරපදය යාවත්කාලීන කරන්න)" icon={<KeyRound size={18} />} />
        <div className="grid sm:grid-cols-2 gap-4 max-w-2xl">
          <div className="space-y-3">
            <Input label={UI_TEXT.Username} value={ownUsername} onChange={(e) => setOwnUsername(e.target.value)} />
            <Input label="New password (නව මුරපදය)" type="password" value={ownNewPw} onChange={(e) => setOwnNewPw(e.target.value)} placeholder="Leave blank to keep current" />
            <Input label="Confirm new password (නව මුරපදය තහවුරු කරන්න)" type="password" value={ownConfirmPw} onChange={(e) => setOwnConfirmPw(e.target.value)} placeholder="Leave blank to keep current" />
            <Button icon={<KeyRound size={15} />} onClick={submitOwnCredentials} disabled={ownSaving}>
              {ownSaving ? 'Saving… (සුරකිමින්…)' : 'Update credentials (අක්තපත්‍ර යාවත්කාලීන කරන්න)'}
            </Button>
          </div>
          <div className="p-4 rounded-xl bg-neutral-50 border border-neutral-100 text-sm space-y-2">
            <div className="font-600 text-neutral-700">Account info (ගිණුම් තොරතුරු)</div>
            <div className="text-neutral-600">Signed in as (ලෙස පිවිසී ඇත): <strong>{user?.displayName}</strong></div>
            <div className="text-neutral-600">Role (භූමිකාව): <strong>{user?.role === 'admin' ? UI_TEXT.Admin : UI_TEXT.DataEntryOperator}</strong></div>
            <div className="text-xs text-neutral-500 mt-2">Update your username or password here. Leave the password fields blank if you only want to change your username. (ඔබේ පරිශීලක නාමය හෝ මුරපදය මෙහි යාවත්කාලීන කරන්න. පරිශීලක නාමය පමණක් වෙනස් කිරීමට අවශ්‍ය නම් මුරපද ක්ෂේත්‍ර හිස්ව තරන්න.)</div>
          </div>
        </div>
      </Card>
      ...
      {isAdmin && (
        <Card className="p-5">
          <SectionTitle
            title={UI_TEXT.UserManagement}
            subtitle="Create, edit, and manage staff accounts (ශ්‍රමික ගිණුම් සාදන්න, සංස්කරණය කරන්න, හා කළමන්කාරකම් කරන්න)"
            icon={<Users size={18} />}
            action={<Button size="sm" icon={<Plus size={14} />} onClick={openCreateUser}>{UI_TEXT.AddUser}</Button>}
          />
          {staffLoading ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs font-600 text-neutral-500 uppercase tracking-wide border-b border-neutral-200">
                    <th className="py-2 px-3">{UI_TEXT.FullName}</th>
                    <th className="py-2 px-3">{UI_TEXT.Username}</th>
                    <th className="py-2 px-3">{UI_TEXT.Role}</th>
                    <th className="py-2 px-3">{UI_TEXT.Status}</th>
                    <th className="py-2 px-3 text-right">{UI_TEXT.Actions}</th>
                  </tr>
                </thead>
                <tbody>
                  {staffUsers.map((u) => (
                    <tr key={u.id} className="border-b border-neutral-100 hover:bg-neutral-50/50 transition">
                      ...
                      <td className="py-2.5 px-3">
                        {u.role === 'admin'
                          ? <Badge tone="green"><ShieldCheck size={12} /> {UI_TEXT.Admin}</Badge>
                          : <Badge tone="neutral"><UserCog size={12} /> {UI_TEXT.DataEntryRole}</Badge>}
                      ...
                      <td className="py-2.5 px-3">
                        {u.status === 'Active'
                          ? <Badge tone="green">{UI_TEXT.ActiveStatus}</Badge>
                          : <Badge tone="red">{UI_TEXT.InactiveStatus}</Badge>}
                      </td>
                      <td className="py-2.5 px-3">
      ...
      {s && (
        <Card className="p-5">
          <SectionTitle title="Custom Categories (අභිරුචි ප්‍රවර්ග)" subtitle="Add or remove categories used across modules (මොඩියුල පුරා භාවිතා කන ප්‍රවර්ග එක් කරන්න හෝ ඉවත් කරන්න)" icon={<Tag size={18} />} />
      ...
      {s && (
        <Card className="p-5">
          <SectionTitle title="Seasons & Years (කන්න හා වර්ෂ)" subtitle="Manage cropping seasons and years for filtering (පෙරහන් සඳහා කන්න හා වර්ෂ කළමන්කාරකම් කරන්න)" icon={<Calendar size={18} />} />
          <div className="grid sm:grid-cols-2 gap-6">
            <div>
              <div className="text-sm font-600 text-neutral-700 mb-2">Seasons (කන්න)</div>
            ...
            <div>
              <div className="text-sm font-600 text-neutral-700 mb-2">Years (වර්ෂ)</div>
              <div className="flex flex-wrap gap-2 mb-3">
      ...
      <Card className="p-5">
        <SectionTitle title="Data Summary (දත්ත සාරාංශය)" subtitle="Records currently stored in the database (දත්ත ගබඩාවේ වර්තමාන සටහන්)" icon={<Database size={18} />} />
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button variant="outline" icon={<Database size={15} />} onClick={exportBackup}>Backup / JSON (උපස්ථ කිරීම)</Button>
          <label>
            <input type="file" accept=".json" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) importBackup(f); e.target.value = ''; }} />
            <span className="inline-flex items-center gap-2 text-sm px-4 py-2.5 rounded-xl bg-white border border-neutral-300 hover:bg-neutral-50 text-neutral-700 font-600 cursor-pointer"><Database size={15} /> Restore backup (උපස්ථ ප්‍රතිස්ථාපනය)</span>
          </label>
        </div>
      </Card>

      <Card className="p-5 border-error-200">
        <SectionTitle title="Danger Zone (අනතුරුදායක කලාපය)" subtitle="Clear all data and start fresh (සියලු දත්ත මකා නැවත ආරම්භ කරන්න)" icon={<AlertTriangle size={18} />} />
        <div className="flex items-center justify-between p-4 rounded-xl bg-error-50 border border-error-200">
          <div>
            <div className="font-600 text-error-800 text-sm">Clear all data (සියලු දත්ත මකන්න)</div>
            <div className="text-xs text-error-700/80">This wipes all records and starts the app with empty data. Consider backing up first. (මෙය සියලු සටහන් මකා යන අතර යෙදුම හිස් දත්ත සමඟ ආරම්භ කරයි. පළමුව උපස්ථ කිරීම සලකා බලන්න.)</div>
          </div>
          <Button variant="danger" icon={<RotateCcw size={15} />} onClick={() => setConfirmReset(true)}>Reset</Button>
        </div>
      </Card>

      <Modal open={confirmReset} onClose={() => setConfirmReset(false)} title="Reset all data? (සියලු දත්ත යළි සැකසීම?)" size="sm">
        <p className="text-sm text-neutral-700">This will permanently delete all records and start the app with empty data. This cannot be undone. (මෙය සියලු සටහන් ස්ථිරව මකා යෙදුම හිස් දත්ත සමඟ ආරම්භ කරයි. මෙය ආපසු අවලංගු කළ නොහැක.)</p>
        <div className="flex justify-end gap-2 mt-5">
          <Button variant="ghost" onClick={() => setConfirmReset(false)}>{UI_TEXT.Cancel}</Button>
          <Button variant="danger" onClick={doReset}>Yes, reset (ඔව්, යළි සැකසුම්)</Button>
        </div>
      </Modal>

      {/* User create/edit modal */}
      <Modal open={userModalOpen} onClose={() => setUserModalOpen(false)} title={editingUser ? 'Edit User (පරිශීලක සංස්කරණය)' : 'Add New User (නව පරිශීලක එක් කරන්න)'} size="md">
        <div className="space-y-4">
          <Input label={UI_TEXT.FullName} value={formFullName} onChange={(e) => setFormFullName(e.target.value)} placeholder="e.g. John Silva" />
          <Input label={UI_TEXT.Username} value={formUsername} onChange={(e) => setFormUsername(e.target.value)} placeholder="e.g. jsilva" />
          <Input label={UI_TEXT.Password} type="password" value={formPassword} onChange={(e) => setFormPassword(e.target.value)} placeholder="Enter password" />
          <div className="grid grid-cols-2 gap-4">
            <Select label={UI_TEXT.Role} value={formRole} onChange={(e) => { const r = e.target.value as 'admin' | 'dataentry'; setFormRole(r); if (r === 'admin') setFormPermissions(defaultPermissionsForRole('admin')); }}>
              <option value="admin">{UI_TEXT.Admin}</option>
              <option value="dataentry">{UI_TEXT.DataEntryRole}</option>
            </Select>
            <Select label={UI_TEXT.Status} value={formStatus} onChange={(e) => setFormStatus(e.target.value as 'Active' | 'Inactive')}>
              <option value="Active">{UI_TEXT.ActiveStatus}</option>
              <option value="Inactive">{UI_TEXT.InactiveStatus}</option>
            </Select>
          </div>
          {formRole === 'dataentry' && (
            <div className="pt-2">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-600 text-neutral-700">{UI_TEXT.Permissions}</span>
              </div>
              <div className="border border-neutral-200 rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-neutral-50 border-b border-neutral-200 text-xs font-600 text-neutral-500 uppercase">
                      <th className="text-left py-2 px-3">Module (මොඩියුලය)</th>
              ...
              <p className="text-xs text-neutral-500 mt-1">Controls what this user can see and edit. Admins always have full access. (මෙම පරිශීලකයාට බලා ගත හැකි හා සංස්කරණය කළ හැකි දේ පාලනය කරයි. පරිපාලකයින්ට සැමවිටම පූර්ණ ප්‍රවේශය ඇත.)</p>
            </div>
          )}
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="ghost" onClick={() => setUserModalOpen(false)}>{UI_TEXT.Cancel}</Button>
            <Button onClick={submitUser} disabled={userSaving}>
              {userSaving ? 'Saving… (සුරකිමින්…)' : editingUser ? UI_TEXT.SaveChanges : 'Create user (පරිශීලක සාදන්න)'}
            </Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Remove user? (පරිශීලක ඉවත් කරන්න?)" size="sm">
        <p className="text-sm text-neutral-700">
          Are you sure you want to remove <strong>{confirmDelete?.full_name}</strong> ({confirmDelete?.username})? This action cannot be undone. (ඔබට මෙම පරිශීලකයා ඉවත් කිරීමට විශ්වාසද? මෙම ක්‍රියාව ආපසු අවලංගු කළ නොහැක.)
        </p>
        <div className="flex justify-end gap-2 mt-5">
          <Button variant="ghost" onClick={() => setConfirmDelete(null)}>{UI_TEXT.Cancel}</Button>
          <Button variant="danger" onClick={confirmDeleteUser}>Yes, remove (ඔව්, ඉවත් කරන්න)</Button>
        </div>
      </Modal>
    </div>
  );
