...
export function FinanceModule() {
  ...
  const allTabs: { key: Tab; label: string }[] = [
    { key: 'ledger', label: UI_TEXT.LedgerEntries },
    { key: 'vouchers', label: UI_TEXT.VouchersBilling },
    { key: 'expenses', label: UI_TEXT.ExpenseLog },
    { key: 'development', label: UI_TEXT.DevelopmentCosts },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat label="Owner capital (මූලික ප්‍රාග්ධනය)" value={LKR(cap.in)} sub={`Drawings ${LKR(drawings)}`} tone="green" icon={<Wallet size={18} />} />
        <Stat label="Bank loan outstanding (වැඩි ණය ඉතිරි)" value={LKR(loan.net)} sub={`Received ${LKR(loan.in)} · Paid ${LKR(loan.out)}`} tone="red" icon={<Landmark size={18} />} />
        <Stat label="Shop transfers (කඩයේ හුවමාරු)" value={LKR(shop.in)} sub="From retail shop entity" tone="blue" icon={<Store size={18} />} />
        <Stat label="Shop credit settled (කඩයේ ණය ගෙවා ඇත)" value={LKR(shopCredit.out)} sub="Settlements to date" tone="yellow" icon={<UserCog size={18} />} />
      </div>
      ...
      {activeTab === 'ledger' && (canView('finance') ? (
        <Card className="p-5">
          <SectionTitle title={UI_TEXT.CapitalFundingLedger} subtitle="Owner capital, retail shop transfers, loans & settlements (මූලික ප්‍රාග්ධනය, ලිල්ලා කඩය හුවමාරු, ණය හා ගෙවීම්)" icon={<BookOpen size={18} />}
            action={<div className="flex gap-2">
              ...
              {tabCanEdit && <Button size="sm" icon={<Plus size={14} />} onClick={() => setModal({ kind: 'ledger' })}>{UI_TEXT.AddEntry2}</Button>}
            </div>} />
          <DataTable
            rows={data.ledger}
            ...
            canEdit={tabCanEdit}
            columns={[
        { key: 'date', header: UI_TEXT.Date, render: (l) => l.date },
        { key: 'kind', header: 'Account (ගිණුම)', render: (l) => l.kind },
        { key: 'dir', header: UI_TEXT.Direction, render: (l) => l.dir },
        { key: 'desc', header: UI_TEXT.Description, render: (l) => l.description },
        { key: 'amount', header: UI_TEXT.Amount, align: 'right', render: (l) => l.amount },
      ]}
    />
  </Card>
) : null)}
      ...
      {activeTab === 'vouchers' && (canView('vouchers') ? (
        <Card className="p-5">
          <SectionTitle title={UI_TEXT.VoucherBillingSystem} subtitle="Every expense, payout, sale & loan settlement generates a voucher (සෑම වියදමක්, ගෙවීමක්, විකුණුමක් හා ණය ගෙවීමක් වවුචරයක් උත්පාදනය කරයි)" icon={<Printer size={18} />}
            action={tabCanEdit && <Button size="sm" icon={<Plus size={14} />} onClick={() => setModal({ kind: 'voucher' })}>{UI_TEXT.NewVoucher}</Button>} />
          <DataTable
            rows={data.vouchers}
            ...
            columns={[
              ...
              { key: 'kind', header: UI_TEXT.Type, render: (v) => <Badge tone={v.kind === 'Sales Receipt' ? 'green' : v.kind === 'Loan Settlement' ? 'red' : v.kind === 'Payroll' ? 'yellow' : 'blue'}>{v.kind}</Badge> },
              { key: 'party', header: UI_TEXT.Party, render: (v) => v.party },
              { key: 'desc', header: UI_TEXT.Description, render: (v) => v.description },
              { key: 'amount', header: UI_TEXT.Amount, align: 'right', render: (v) => <span className="font-700">{LKR(v.amount)}</span> },
              { key: 'act', header: UI_TEXT.Actions, render: (v) => (
      ...
      {activeTab === 'expenses' && (canView('expenses') ? (
        <Card className="p-5">
          <SectionTitle title={UI_TEXT.ExpenseLog} subtitle={UI_TEXT.ExpenseLogSubtitle} icon={<Wallet size={18} />}
            action={tabCanEdit && <Button size="sm" icon={<Plus size={14} />} onClick={() => setModal({ kind: 'expense' })}>{UI_TEXT.AddExpense2}</Button>} />
          <DataTable
            rows={data.expenses}
            ...
            columns={[
              ...
              { key: 'class', header: 'Class (වර්ගය)', render: (e) => <Badge tone="blue">{e.class}</Badge> },
              { key: 'cat', header: UI_TEXT.Category, render: (e) => e.category || <span className="text-error-500">— missing —</span> },
              { key: 'desc', header: UI_TEXT.Description, render: (e) => e.description || <span className="text-error-500">— missing —</span> },
              { key: 'amount', header: UI_TEXT.Amount, align: 'right', render: (e) => <span className="font-700">{LKR(e.amount)}</span> },
              { key: 'act', header: UI_TEXT.Actions, render: (e) => (
      ...
      {activeTab === 'development' && (canView('capex') ? (
        <Card className="p-5">
          <SectionTitle title={UI_TEXT.FarmInfrastructure} subtitle={UI_TEXT.CAPEXSubtitle} icon={<Hammer size={18} />}
            action={<div className="flex gap-2">
              ...
              {tabCanEdit && <Button size="sm" icon={<Plus size={14} />} onClick={() => setModal({ kind: 'development' })}>{UI_TEXT.AddDevelopment}</Button>}
            </div>} />
          <DataTable
            ...
            canEdit={tabCanEdit}
            columns={[
              { key: 'name', header: UI_TEXT.AssetDevelopmen , render: (d) => <div><div className="font-600">{d.name}</div><div className="text-xs text-neutral-500">{d.description || '—'}</div></div> },
              { key: 'cat', header: UI_TEXT.Category, render: (d) => <Badge tone="blue">{d.category}</Badge> },
              { key: 'date', header: UI_TEXT.Implemented, render: (d) => fmtDate(d.implementationDate) },
              { key: 'lifespan', header: UI_TEXT.Lifespan, align: 'right', render: (d) => `${d.lifespanYears} yr` },
              { key: 'plot', header: UI_TEXT.LinkedPlot, render: (d) => { const c = data.crops.find((x) => x.id === d.linkedPlotId); return c ? c.name : '—'; } },
              { key: 'cost', header: UI_TEXT.TotalCostLabel, align: 'right', render: (d) => <span className="font-700 text-primary-700">{LKR(d.totalCost)}</span> },
            ]}
          />
        </Card>
      ) : null)}
