  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl shadow-card-lg">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-6 p-6 lg:p-8 text-white">
          ...
          <div className="grid grid-cols-2 gap-3 lg:gap-4 shrink-0">
            <HeroStat label="Revenue" value={LKR(overall.revenue)} />
            <HeroStat label="Net profit" value={LKR(overall.profit)} tone="yellow" />
            <HeroStat label="Nursery sales" value={LKR(nur.salesRevenue)} />
            <HeroStat label="Loan outstanding" value={LKR(loan.net)} tone="yellow" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat label="Maha profit" value={LKR(maha.profit)} sub={`${maha.revenue ? ((maha.profit / maha.revenue) * 100).toFixed(1) : 0}% margin`} tone="green" icon={<TrendingUp size={18} />} />
        <Stat label="Yala profit" value={LKR(yala.profit)} sub={`${yala.revenue ? ((yala.profit / yala.revenue) * 100).toFixed(1) : 0}% margin`} tone="blue" icon={<TrendingUp size={18} />} />
        <Stat label="Payroll (this month)" value={LKR(pay.total)} sub={`Perm ${LKR(pay.permanent)} · Casual ${LKR(pay.casual)}`} tone="yellow" icon={<Users size={18} />} />
        <Stat label="Capital injected" value={LKR(cap.in)} sub={`Owner drawings ${LKR(ledgerBalance(data, 'Owner Equity Return').out)}`} tone="neutral" icon={<Wallet size={18} />} />
      </div>

      {/* Revenue vs Expenses + Crop yields */}
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="p-5">
          <SectionTitle title="Revenue vs Expenses" subtitle="Total income versus total costs across all operations" icon={<BarChart3 size={18} />} />
          <div className="space-y-4 mt-2">
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-sm font-600 text-neutral-700 flex items-center gap-1.5"><TrendingUp size={14} className="text-success-600" /> Total Revenue</span>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-sm font-600 text-neutral-700 flex items-center gap-1.5"><TrendingDown size={14} className="text-error-600" /> Total Expenses</span>
            ...
            <div className="pt-2 border-t border-neutral-100 flex items-center justify-between">
              <span className="text-sm text-neutral-600">Net profit margin</span>
        ...
        <Card className="p-5">
          <SectionTitle title="Crop Yield (kg harvested)" subtitle="Top crops by harvest volume" icon={<Leaf size={18} />} action={<Button size="sm" variant="ghost" onClick={() => onNavigate('crops')}>View all</Button>} />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="p-5">
          <SectionTitle title="Development CAPEX" subtitle="Capital expenditure & depreciation" icon={<Hammer size={18} />} action={<Button size="sm" variant="ghost" onClick={() => onNavigate('finance')}>Open</Button>} />
          <div className="space-y-3 mt-2">
            <Row label="Total CAPEX" value={LKR(totalCapex)} />
            <Row label="Projects" value={String(data.farmDevelopments.length)} />
            <Row label="Annual depreciation" value={LKR(Math.round(annualDepreciation))} tone="green" />
          </div>
        ...
        <Card className="p-5 lg:col-span-2">
          <SectionTitle title="Top crops by profit" subtitle="Across all seasons" icon={<Leaf size={18} />} action={<Button size="sm" variant="ghost" onClick={() => onNavigate('crops')}>View all</Button>} />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="p-5">
          <SectionTitle title="System Alerts" subtitle="Items needing your attention" icon={<AlertTriangle size={18} />} />
          <div className="space-y-2 mt-2">
            {alerts.length === 0 ? (
              <div className="flex items-center gap-2 p-3 rounded-xl bg-success-50 text-sm text-success-700">
                <CheckCircle2 size={16} /> All systems normal — no alerts.
              </div>
            ) : alerts.map((a, i) => (
        ...
        <Card className="p-5">
          <SectionTitle title="Nursery at a glance" icon={<Sprout size={18} />} action={<Button size="sm" variant="ghost" onClick={() => onNavigate('nursery')}>Open</Button>} />
          <div className="space-y-3">
            <Row label="Batches" value={String(data.nurseryBatches.length)} />
            <Row label="Ready units" value={String(data.nurseryBatches.filter((b) => b.status === 'Ready').reduce((s, b) => s + b.qtyUnits, 0))} />
            <Row label="Sales revenue" value={LKR(nur.salesRevenue)} />
            <Row label="Transfer credit" value={LKR(nur.transferCredit)} />
            <Row label="Net profit" value={LKR(nur.netProfit)} tone="green" />
          </div>
...
function DataEntryDashboard({ data, user, onNavigate }: { data: ReturnType<typeof useStore>['data']; user: { displayName: string } | null; onNavigate: (k: ModuleKey) => void }) {
  ...
  const tasks: { label: string; done: boolean }[] = [
    { label: 'Record today\'s attendance', done: todaysAttendance.length > 0 },
    { label: 'Log any new nursery costs', done: data.nurseryCosts.some((c) => c.date === today) },
    { label: 'Add crop expenses for today', done: data.cropExpenses.some((e) => e.date === today) },
    { label: 'Record any harvest sales', done: data.cropHarvests.some((h) => h.date === today) },
    { label: 'Update nursery batch statuses', done: growingBatches.length === 0 || readyBatches.length > 0 },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl shadow-card-lg">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-6 p-6 lg:p-8 text-white">
          <div className="flex items-start gap-4">
            ...
            <div>
              <Badge tone="green" className="bg-white/20 text-white">Data Entry Operator</Badge>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <Button variant="secondary" size="sm" icon={<ClipboardList size={15} />} onClick={() => onNavigate('dataentry')}>Go to Data Entry</Button>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 lg:gap-4 shrink-0">
            <HeroStat label="Nursery batches" value={String(data.nurseryBatches.length)} />
            <HeroStat label="Attendance" value={String(data.attendance.length)} />
            <HeroStat label="Crop expenses" value={String(data.cropExpenses.length)} />
            <HeroStat label="Total records" value={String(recentEntries)} tone="yellow" />
          </div>
        </div>
      </div>
      ...
      <Card className="p-5">
        <SectionTitle title="Quick Actions" subtitle="Jump straight to common data entry tasks" icon={<Plus size={18} />} />
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2">
          <QuickAction icon={<Wallet size={20} />} label="+ New Expense" color="bg-error-50 text-error-600 border-error-200" onClick={() => onNavigate('dataentry')} />
          <QuickAction icon={<Leaf size={20} />} label="+ New Harvest" color="bg-success-50 text-success-600 border-success-200" onClick={() => onNavigate('dataentry')} />
          <QuickAction icon={<Users size={20} />} label="+ Attendance" color="bg-secondary-50 text-secondary-600 border-secondary-200" onClick={() => onNavigate('dataentry')} />
          <QuickAction icon={<Sprout size={20} />} label="+ Nursery Batch" color="bg-primary-50 text-primary-600 border-primary-200" onClick={() => onNavigate('dataentry')} />
        </div>
      </Card>

      {/* KPI grid — operational, no financials */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat label="Nursery entries" value={String(data.nurseryBatches.length)} sub="Total batches" tone="green" icon={<Sprout size={18} />} />
        <Stat label="Labor entries" value={String(data.attendance.length)} sub="Attendance records" tone="yellow" icon={<Users size={18} />} />
        <Stat label="Crop expenses" value={String(data.cropExpenses.length)} sub="Field inputs logged" tone="blue" icon={<Leaf size={18} />} />
        <Stat label="Today's attendance" value={String(todaysAttendance.length)} sub="Records logged today" tone="neutral" icon={<CalendarDays size={18} />} />
      </div>

      <div className="grid lg:grid-cols-3 gap-6">
        {/* Daily Task Checklist */}
        <Card className="p-5">
          <SectionTitle title="Daily Task Checklist" subtitle={`${completedTasks}/${tasks.length} completed`} icon={<CheckCircle2 size={18} />} />
          <div className="space-y-2 mt-2">
            {tasks.map((t, i) => (
        ...
        {/* Recent Entries */}
        <Card className="p-5 lg:col-span-2">
          <SectionTitle title="Recent Entries" subtitle="Your latest data submissions" icon={<ClipboardList size={18} />} action={<Button size="sm" variant="ghost" onClick={() => onNavigate('dataentry')}>Add more</Button>} />
          <div className="space-y-2">
            ...
            {recentEntries === 0 && <div className="text-sm text-neutral-400 py-4 text-center">No entries yet. Use Quick Actions to get started.</div>}
          </div>
        </Card>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="p-5">
          <SectionTitle title="Nursery Status" subtitle="Batch production overview" icon={<Sprout size={18} />} />
          <div className="grid grid-cols-3 gap-3 mt-2">
            <StatusCard label="Growing" count={growingBatches.length} tone="yellow" icon={<Sprout size={16} />} />
            <StatusCard label="Ready" count={readyBatches.length} tone="green" icon={<CheckCircle2 size={16} />} />
            <StatusCard label="Total" count={data.nurseryBatches.length} tone="blue" icon={<ClipboardList size={16} />} />
          </div>
          <div className="mt-3 space-y-1.5">
            ...
            {data.nurseryBatches.length === 0 && <div className="text-xs text-neutral-400">No batches yet</div>}
          </div>
        </Card>

        <Card className="p-5">
          <SectionTitle title="Crop Plot Status" subtitle="Active field plots" icon={<Leaf size={18} />} />
          <div className="grid grid-cols-3 gap-3 mt-2">
            <StatusCard label="Active" count={activeCrops.length} tone="green" icon={<Leaf size={16} />} />
            <StatusCard label="Harvested" count={data.crops.filter((c) => c.status === 'Harvested').length} tone="blue" icon={<CheckCircle2 size={16} />} />
            <StatusCard label="Total" count={data.crops.length} tone="yellow" icon={<ClipboardList size={16} />} />
          </div>
          <div className="mt-3 space-y-1.5">
            ...
            {activeCrops.length === 0 && <div className="text-xs text-neutral-400">No active crops</div>}
          </div>
        </Card>
      </div>
      ...
      <Card className="p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="font-600 text-sm text-neutral-800">Your access level: Data Entry</div>
