...
export function mapImport(target: ImportTarget, rows: Record<string, string>[]): unknown[] {
  ...
  return rows.map((r, i) => {
    switch (target) {
      ...
      case 'attendance': return { id: id(i, 'at'), workerId: r.workerId || '', date: r.date || '', status: (r.status || 'Present') as 'Present' | 'Absent' | 'Half Day', taskPlot: r.taskPlot || '', hours: num(r.hours), amount: num(r.amount), overrideRate: r.overrideRate != null ? num(r.overrideRate) : undefined, fuelTransportAllowance: num(r.fuelTransportAllowance), attendanceAllowance: num(r.attendanceAllowance), otherAllowances: num(r.otherAllowances), fuelAllocation: (r.fuelAllocation || undefined) as 'CROP' | 'OVERHEAD' | undefined, fuelCropId: r.fuelCropId || undefined, expenseAllocation: (r.allocationType || r.allocation_type) ? { allocationType: (r.allocationType || r.allocation_type) as 'CROP' | 'FARM_DEVELOPMENT', cropId: (r.cropId || r.crop_id) || undefined, plotId: (r.plotId || r.plot_id) || undefined, developmentCategory: (r.developmentCategory || r.development_category) || undefined } : undefined };
